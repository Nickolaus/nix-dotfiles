...
- `codebase-memory-mcp` is built from its pinned upstream flake and declared in `aiAgents`, but kept out of native/global MCP registration because upstream's default cache is intentionally cross-repo. Repos that want it should use `codebase-memory-onboard <repo>` to register Codex with a scoped `CBM_CACHE_DIR = ".codex/cache/codebase-memory"` while keeping `.codex/config.toml` and the cache private to that clone via `.git/info/exclude` or `--skip-worktree`; do not commit personal MCP setup to shared repos. Once visible, it is the zero-token structural graph for call graphs, dead code, Cypher queries, and git-diff impact; prefer it over grep for "who calls X" / "what breaks if I change Y" questions. Graphify (below) remains the tool for deep, cross-document (code + docs + papers + media) architecture exploration invoked on demand.
