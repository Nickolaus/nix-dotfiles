...
- `codebase-memory-mcp` is built from its pinned upstream flake and exposed through `aiAgents` for Codex, Claude Code, Cursor, and Vibe. It's a zero-token, always-on structural code graph (call graphs, dead code, Cypher queries, git-diff impact) — prefer it over grep for "who calls X" / "what breaks if I change Y" questions on indexed repos. Graphify (below) remains the tool for deep, cross-document (code + docs + papers + media) architecture exploration invoked on demand.
- `codebase-memory-mcp` defaults to `auto_index=true` (opt-out via `config set auto_index false`, asserted idempotently on every switch via `home.activation` since the setting lives in a mutable local SQLite config db, not a file Nix can own) — new repos index transparently on first MCP connection, no explicit tool call needed. Persists its graph at `~/.cache/codebase-memory-mcp/`; run `codebase-memory-status` for live config/project state. It also contributes a short "use the graph, not broad reads" section to the same global `.codex/AGENTS.md`/`.claude/CLAUDE.md`/`.vibe/AGENTS.md` that `caveman.nix` writes — Home Manager merges both via `types.lines`, each feature owning its own section.
