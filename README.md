...
- `codebase-memory-mcp` is built from its pinned upstream flake and declared in `aiAgents`, but kept out of native/global MCP registration because upstream's default cache is intentionally cross-repo. Repos that want it should use `codebase-memory-onboard <repo>` to register Codex with a scoped `CBM_CACHE_DIR = ".codex/cache/codebase-memory"` while keeping `.codex/config.toml` and the cache private to that clone via `.git/info/exclude`; do not commit personal MCP setup to shared repos. If `.codex/config.toml` is already tracked, onboarding refuses instead of hiding local edits with `--skip-worktree`, because rebases and branch switches can still need to update tracked files. Once visible, it is the zero-token structural graph for call graphs, dead code, Cypher queries, and git-diff impact; prefer it over grep for "who calls X" / "what breaks if I change Y" questions. Graphify (below) remains the tool for deep, cross-document (code + docs + papers + media) architecture exploration invoked on demand.
- Repo-scoped `codebase-memory-mcp` caches should keep `auto_index=true` so new repos index transparently on first MCP connection. The global upstream cache remains separate and may be empty or have `auto_index=false`; that does not mean a repo-scoped Codex setup is broken. Run `codex-ai-status` from a repo to inspect active Codex MCP config, scoped codebase-memory state, and Graphify presence; run `codebase-memory-status` when you need both global and current-repo cache details. It also contributes a short "use the graph, not broad reads" section to the same global `.codex/AGENTS.md`/`.claude/CLAUDE.md`/`.vibe/AGENTS.md` that `caveman.nix` writes — Home Manager merges both via `types.lines`, each feature owning its own section.
