...
- **Codex and Vibe's default routing is fully declarative, no wrapper needed** — both tools' own native base-URL override mechanisms point at a proxy unconditionally for compression, so the plain `codex`/`vibe` commands are compressed automatically with no auth-model change. **OpenCode's local-Ollama route is opt-in** as a real, baked-in named provider (no separate command). **Claude Code has no local-Ollama route at all** — its config schema has no concept of multiple named providers, so switching providers always means different `ANTHROPIC_*` values at launch; a per-invocation wrapper for that would silently compete with any other app (e.g. an IDE) also pointed at the same local-coding backend, with no visibility into who was holding the single-request-at-a-time Ollama slot (see below).
