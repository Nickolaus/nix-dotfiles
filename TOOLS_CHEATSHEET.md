...
OpenCode needs no wrapper at all: a `local-ollama` provider is merged natively into `~/.config/opencode/opencode.json` (same shape as Codex's `model_providers.local_coding_ollama`) -- pick it from OpenCode's own `/models` picker or `opencode --model local-ollama/<id>`.
...
- Two integration layers, both declarative:
  ...
  2. **A declarative registry of persistent local proxies** (`headroom proxy`, `home/features/ai/headroom.nix`) — launchd agents (macOS only) that start at login and stay up, compressing all traffic routed through them before forwarding to their configured upstream. A single `headroom proxy` instance only forwards OpenAI-shaped traffic to *one* `OPENAI_TARGET_API_URL` (and Anthropic-shaped traffic to *one* upstream), so any two tools needing genuinely different upstreams need their own instance. `headroom.nix` itself has **zero per-tool knowledge** — it's a generic engine that just spins up whatever's declared in `aiAgents.headroom.proxies` (`hosts/shared/ai-agents.nix`), so onboarding a future provider needing its own upstream is exactly one new attrset entry there, never a code change in `headroom.nix`. Today's entries:
     ...
     | --- | --- | --- | --- | --- |
     | `shared` | `org.nix-community.home.headroom-proxy-shared` | 8787 | real Anthropic route → local Ollama; real OpenAI route → real OpenAI | Claude Code, Codex |
...
- **Why no separate "gateway" tool (LiteLLM/Portkey/etc.) was needed**: every one of these tools speaks either Anthropic Messages (Claude Code) or an OpenAI-compatible wire format (everyone else, including Mistral Cloud) — and the Headroom proxy already speaks both on the same port (`/v1/messages`, `/v1/chat/completions`, `/v1/responses`). It **is** the protocol-agnostic gateway; no extra layer needed. What differs per tool is only how each one is told to point at it:
  ...
  | Codex | `openai_base_url` in the managed `config.toml` | yes — keeps ChatGPT sign-in *or* API key on the built-in `openai` provider | **on by default** (compression only; local Ollama is a separate, explicitly-selected `model_providers.local_coding_ollama` entry) |
  ...
  | OpenCode | `provider.local-ollama` in `opencode.json`, merged in natively (same shape as Codex's named provider — the local-coding route is always exactly one pinned model, so no live catalog generation is needed) | n/a — separate named provider, doesn't touch any other provider's auth | opt-in, no wrapper: pick `local-ollama` from `/models` or `--model` |
  ...
  Any other GUI app/IDE extension (VS Code, Continue, Cline, ...) can opt in the same way via the session-wide `LOCAL_CODING_ANTHROPIC_BASE_URL`/`LOCAL_CODING_OPENAI_BASE_URL`/`LOCAL_CODING_API_KEY`/`LOCAL_CODING_MODEL` env vars (`launchctl setenv`, set at login by `home/features/ai/headroom.nix`) — deliberately non-reserved names so nothing auto-detects them; each tool points its own Base URL/API key setting at them manually. Current values: `headroom-status`.
- **Codex and Vibe's default routing is fully declarative, no wrapper needed** — both tools' own native base-URL override mechanisms point at a proxy unconditionally for compression, so the plain `codex`/`vibe` commands are compressed automatically with no auth-model change. **OpenCode's local-Ollama route is opt-in** as a real, baked-in named provider (no separate command). **Claude Code has no local-Ollama route at all** — its config schema has no concept of multiple named providers, so switching providers always means different `ANTHROPIC_*` values at launch; a wrapper for that was tried and dropped (see below), since it silently competed with any other app (e.g. an IDE) also pointed at the same local-coding backend, with no visibility into who was holding the single-request-at-a-time Ollama slot.
...
- **OpenCode's `local-ollama` provider is baked into `~/.config/opencode/opencode.json` declaratively** (`home/features/ai/headroom.nix`, `home.activation.mergeOpencodeLocalProvider`), the same shape as Codex's `model_providers.local_coding_ollama` — a `jq`-based merge that only ever touches the `provider.local-ollama` key, so any other provider or setting you (or `opencode auth login`) add to that file is left alone. Its custom-provider config needs every model listed explicitly (no wildcard), but the local-coding route is always exactly one pinned model (`aiAgents.localCoding.model`), so a static entry is all that's needed — no live-generated catalog, no wrapper binary. (An earlier `headroom-opencode` wrapper that shelled out to `headroom wrap opencode` for a live-generated catalog has been retired in favor of this.)
- **Claude Code has no local-coding route, by decision.** A `claude-local` wrapper (same shape as Codex's `model_providers.local_coding_ollama`) was built and then removed: Claude Code's config schema has no multi-provider concept at all, so a wrapper exporting `ANTHROPIC_BASE_URL`/`AUTH_TOKEN`/`API_KEY`/`MODEL` for one invocation was the only way to do it without touching the default `claude` command's auth. In practice it turned out to share the same single-loaded-model, single-request-at-a-time Ollama backend with any other app opted into the `LOCAL_CODING_*` route (e.g. an IDE's own agent feature) with zero visibility into who else was using it — one app's slow request silently starved another's. Given OpenCode already covers free/local coding natively with none of that risk, the wrapper was dropped rather than fixed.
