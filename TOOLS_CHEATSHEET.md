...
- `context7`, `fetch`, `sequential-thinking`, `time`, `github`, and `headroom` are the universal baseline (`defaultProfileServers` in `hosts/shared/ai-agents.nix`) and stay natively registered for all 4 clients. `codebase-memory` is also natively registered (for Codex/Claude/Vibe, not Cursor) via a wrapper command that resolves a per-repo `CBM_CACHE_DIR` from the git root at spawn time, avoiding global cross-repo graph bleed without any per-repo config.
...
- The shared `aiAgents` MCP config natively registers it for Codex/Claude/Vibe. `command` points at `codebaseMemoryScopedWrapper` (`hosts/shared/ai-agents.nix`), not the raw binary — no per-repo onboarding or project MCP config needed.
- Zero LLM tokens for indexing: pure tree-sitter + Hybrid LSP static analysis in a single C binary. Upstream's default graph cache is `~/.cache/codebase-memory-mcp/`; the wrapper instead resolves `~/.cache/codebase-memory-mcp/repos/<slug>-<hash>/` from `git rev-parse --show-toplevel` at spawn time and refuses to start outside a git repo.
- The wrapper sets `auto_index=true` on every start. The global upstream cache may be empty or have `auto_index=false`; that does not mean the current repo's MCP is broken — check the resolved per-repo cache instead (`codex-ai-status` / `codebase-memory-status`).
...
- The wrapper never writes into any repo file (tracked or untracked), so it works identically whether a repo ships its own `.codex/config.toml` or has none, with no `git rebase`/checkout risk (`docs/codex-mcp-config-rebase-research-2026.md` covers why that matters).
