...
headroom-opencode [-- args]     # Run OpenCode through the shared proxy (real API, compressed) -- still opt-in
```
...
- **Why no separate "gateway" tool (LiteLLM/Portkey/etc.) was needed**: every one of these tools speaks either Anthropic Messages (Claude Code) or an OpenAI-compatible wire format (everyone else, including Mistral Cloud) — and the Headroom proxy already speaks both on the same port (`/v1/messages`, `/v1/chat/completions`, `/v1/responses`). It **is** the protocol-agnostic gateway; no extra layer needed. What differs per tool is only how each one is told to point at it:
  ...
  | --- | --- | --- | --- |
  | Claude Code | `ANTHROPIC_BASE_URL` env var (managed-settings.json *and* user settings.json) | yes | **on by default** |
  | Codex | `openai_base_url` in the managed `config.toml` | yes — keeps ChatGPT sign-in *or* API key on the built-in `openai` provider | **on by default** |
  | Vibe | `api_base` on the "mistral" provider entry, guarded `tomlkit` rewrite in `~/.vibe/config.toml` | yes — same `MISTRAL_API_KEY`/`Authorization: Bearer` passthrough | **on by default**, opt out with `headroom-pause vibe` |
  | OpenCode | `provider.<id>.options.baseURL` in `opencode.json` | via `headroom wrap opencode` (dynamic model catalog) | opt-in (`headroom-opencode`) |
...
- **Claude, Codex, and Vibe's default routing is fully declarative, no wrapper needed.** There is no way to compress real wire traffic to a closed-source CLI without a proxy in the path — Headroom's own docs confirm every integration surface other than "you own the code calling the LLM SDK" (which none of these CLIs let you do) ultimately funnels through `headroom proxy`. What *is* avoidable is treating that proxy as opt-in: all three tools' own native base-URL override mechanisms now point at a proxy unconditionally, so the plain `claude`/`codex`/`vibe` commands are compressed automatically — no `headroom wrap`/`headroom-*` subprocess, no separate `--profile`.
  - **Claude** already defaulted to a local, free Ollama backend (`localAi`) rather than real Anthropic. Routing it through Headroom first would have silently swapped that for real, paid Anthropic — so the shared proxy's own `ANTHROPIC_TARGET_API_URL` (home/features/ai/headroom.nix) is pointed at that *same* local Ollama endpoint. Net effect: identical free/local destination, now compressed.
...
- **OpenCode stays opt-in via `headroom wrap opencode --no-proxy --no-mcp --no-tokensave --no-serena --no-context-tool`** (the `--no-*` flags skip Headroom's own MCP/tokensave/Serena auto-registration, since we already declare those MCP servers ourselves via `aiAgents`). Its custom-provider config needs an explicit, versioned model catalog that `headroom wrap` generates dynamically at launch — hand-maintaining that list in Nix would go stale, so this one path stays imperative rather than durable-by-default.
- **Opting out of default routing**: `headroom-pause` stops every registered proxy; `headroom-pause <name>` (e.g. `headroom-pause vibe`) stops just one — the same, single mechanism for all three tools, no per-tool escape-hatch commands. Since Claude/Codex/Vibe point at their proxies unconditionally, whichever instance you stop will fail to connect until `headroom-resume [name]` (or a rebuild that removes the routing). This mirrors the same dependency risk this machine already accepts for Claude's Ollama routing (`localAi`) — if that's down, Claude's default already fails the same way; Vibe now carries the identical, symmetric trade-off rather than a bespoke "fail gracefully to direct-Mistral" path that only it would have had.
